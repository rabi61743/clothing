...
export default function AtelierAdminDashboard() {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      const [statsRes, ordersRes] = await Promise.all([
        ...
        fetch("http://localhost:8080/api/orders?limit=15"),
      ]);
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-white font-sans selection:bg-white selection:text-black">
      {/* 1. ADMIN HEADER */}
      <header className="border-b border-neutral-800 bg-black/50 backdrop-blur-md sticky top-0 z-40">
      ...
      <main className="max-w-7xl mx-auto px-6 py-10 space-y-10">
        ...
        <div>
          ...
          <h1 className="text-3xl font-black uppercase tracking-tight mt-1">
            Global Flagship Atelier Dashboard
          </h1>
        </div>

        {/* Success Alert */}
        {createdProductSuccess && (
          <div className="bg-emerald-950/60 border border-emerald-800 p-4 rounded-none flex items-center justify-between text-xs">
            <div className="flex items-center gap-2 text-emerald-300">
              <CheckCircle2 className="w-4 h-4 text-emerald-400" />
              <span>
                <strong>{createdProductSuccess}</strong> created successfully with automated 768-d pgvector embedding indexed!
              </span>
            </div>
            <button 
          ...
          <div className="flex items-center justify-between pb-4 border-b border-neutral-800">
            <div>
              ...
              <p className="text-xs text-neutral-500 font-mono mt-0.5">
                Real-time transactional stream from PostgreSQL
              </p>
            </div>
            <span className="text-xs font-mono bg-neutral-800 px-3 py-1 text-neutral-300">
              {orders.length} Records Retrieved
            </span>
          </div>

          {orders.length === 0 ? (
            <div className="text-center py-12 text-neutral-500 text-xs">
              No customer orders placed yet. Visit the checkout page to test live order placement.
            </div>
          ) : (
                ...
                <tbody className="divide-y divide-neutral-800">
                  {orders.map((o) => (
                    <tr key={o.id} className="hover:bg-neutral-800/50 transition-colors">
                      ...
                      <td className="py-4">
                        <span className="bg-emerald-950 text-emerald-400 border border-emerald-800 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider">
                          {o.status}
                        </span>
                      </td>
                    </tr>
